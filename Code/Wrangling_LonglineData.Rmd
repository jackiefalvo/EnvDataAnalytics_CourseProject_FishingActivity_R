...
```{r}
colnames(longline_fishing_recode)

# reorder and rename data frame columns
longline_processed <- 
  select(longline_fishing_recode, mmsi, date, lat, lon, speed, distance_from_shore, distance_from_port, is_fishing)

# separate date and time columns
longline_processed %>% separate(date, c("Date", "Time"), " ")


class(longline_processed$date)
longline_processed$date <- as.Date(longline_processed$date, format = "%Y-%M-%D%H:%M:%S")

colnames(longline_processed)[1] <- ("Vessel")
colnames(longline_processed)[1] <- ("Vessel")
